  ...
  },
{
  ...
  descripcion: 'Full-stack system for hospital management. Includes JWT authentication, automated ticketing, and real-time database updates.',
  imagen: 'url-de-tu-foto-o-video',
  links: [
...
const CardProyecto = ({ titulo, descripcion, imagen, links, tags }) => (
  <div className="bg-white p-6 rounded-xl shadow-lg my-4 transition duration-300 hover:shadow-xl hover:scale-[1.02] flex flex-col h-full">
    <img
      src={imagen}
      alt={titulo}
    ...
    <div className="flex gap-2 mb-3">
      {tags.map(tag => (
        <span key={tag} className="text-[10px] font-bold px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded-md uppercase">
          {tag}
        </span>
      ))}
    </div>
    <h3 className="text-xl font-bold text-gray-800 mb-2">{titulo}</h3>
    <p className="text-gray-600 text-sm mb-6 flex-grow">{descripcion}</p>
    ...
    <div className="flex flex-wrap gap-3 pt-4 border-t border-gray-100">
      {links.map(link => (
        <a 
          ...
          href={link.url}
          target="_blank"
          className="text-xs font-bold flex items-center gap-1 text-indigo-600 hover:text-indigo-800"
        >
          {link.label === 'GitHub' && <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">...</svg>}
          {link.label}
        </a>
      ))}
    </div>
